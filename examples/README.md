...
| Evidence `authorization` (batch authorization JWS) and `merkleProof` | Generated by the pipeline at batch-signing time — they depend on the batch's Merkle root and the authorizer's key, so they are not authored by hand. The source evidence carries only `authorizer`. | `signed/<name>.decoded.json` |

...
| Evidence `authorization` (batch authorization JWS) and `merkleProof` | Generated by the pipeline at batch-signing time — they depend on the batch's Merkle root and the authorizer's key, so they are not authored by hand. The source evidence carries only `authorizedBy`. | `signed/<name>.decoded.json` |
...
authorization JWS and Merkle inclusion proof. See
